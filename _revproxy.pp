...
uses
  winsock, crt, math, dos, sysutils, lexlib, revlib;
        ...
        begin
          ...
          case source of
            ...
            src_host: // handle data from the host
              begin
                yyvars.buflen:=recv(proxy.host_fd, yyvars.buf, sizeof(yyvars.buf), 0);
                yyvars.bufptr:=1;
                if yyvars.buflen<=0 then
                begin
                ...
                begin
                  //display_window(display,WINDOW_USERMSGS);
                  //writeln('packetsize=',yyvars.buflen);
                  //synch:=false;
                  ...
                  begin
                    ...
                    if (fd_isset(current_fd, proxy.socket_fds)) then
                      if (current_fd <> proxy.listener_fd) and (current_fd <> proxy.host_fd) then
                        if (send(current_fd, yyvars.buf, yyvars.buflen, 0) = SOCKET_ERROR) then sock_error(proxy,'send()',display)
                  end;//for
                  //process_host_state(hs);
                  //if yyhostlex(yyvars,display)=0 then ;//write('*');
                  //synch:=insynch(cs,hs);
                end;
              end;
            src_client: // handle data from a client
              begin
                ...
                begin
                  // synch:=false;
                  datastr:=ansistring(cs.buf);
                  setlength(datastr,cs.buf_n);
                  datastr:=translate(datastr,ansistring(#13),'*');
                  //display_window(display,WINDOW_USERMSGS);
                  //writeln('cl: ',datastr);
                  //write('cl(',length(datastr),'):');
                  //for k:=1 to length(datastr) do
                  //  write(ord(datastr[k]):4);
                  //writeln;
